...
Link: ${storeUrl}
Contact: ${contact}
Networks: ${Object.keys(netCount).length > 0 ? Object.keys(netCount).map(n => n.toUpperCase()).join(', ') : 'MTN, AirtelTigo, Telecel'}
Prices:
${priceLines.join('\n') || '  (not set)'}`;
  }
  ...
  async function generate() {
    ...
    // Combine the fixed prompt with the optional short detail
    const finalPrompt = detail.trim()
      ? `${selectedPrompt}. Extra detail: ${detail.trim()}`
      : selectedPrompt;
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {QUICK_PROMPTS.map(q => (
              <button
                key={q.label}
                onClick={() => { setSelectedPrompt(q.value); setMessage(''); setDetail(''); }}
                style={{
                  padding: '6px 12px', borderRadius: 20,
                  border: `1px solid ${selectedPrompt === q.value ? 'rgba(37,211,102,0.5)' : 'var(--border)'}`,
                  background: selectedPrompt === q.value ? 'rgba(37,211,102,0.1)' : 'var(--surface2)',
                  color: selectedPrompt === q.value ? '#25d366' : 'var(--text2)',
        ...
        {/* Optional short detail — only shown after a prompt is selected */}
        {selectedPrompt && (
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label">
              Add a detail <span style={{ color: 'var(--text3)', fontWeight: 400, textTransform: 'none' }}>(optional · max 60 characters)</span>
            </label>
            <div style={{ position: 'relative' }}>
              <input
                className="form-input"
                placeholder="e.g. mention 5GB MTN at ₵19 this weekend only"
                maxLength={60}
                value={detail}
              ...
              <span style={{
                ...
                fontSize: 11, color: detail.length >= 55 ? 'var(--warn)' : 'var(--text3)',
        ...
        {selectedPrompt && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            >
              {loading ? <><span className="spinner" /> Writing…</> : <>✨ Generate</>}
            </button>
          </div>
        )}
